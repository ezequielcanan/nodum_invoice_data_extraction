...
class Database():
  ...
  def make_db_action(self, func, logger: Log, *args):
    if not(logger is None):
      logger.debug(f"DB, Log no nulo")
    conn = 0
    if isEnvDev:
    ...
    cur = conn.cursor()
    if not(logger is None):
      logger.debug(f"DB, Conectado a DB")
    result = func(cur, logger, *args)
    if not(logger is None):
      logger.debug(f"DB, Funcion Ejecutada")
    conn.commit()
    conn.close()
    if not(logger is None):
      logger.debug(f"DB, Cierra Conexion")
    return result

  def get_emails(self, cur, logger: Log = None):
    if not(logger is None):
      logger.debug(f"SELECT * FROM emails")
    cur.execute("SELECT * FROM emails")
    return cur.fetchall()
  
  def get_email(self, cur, logger: Log, email):
    if not(logger is None):
      logger.debug(f"SELECT * FROM emails WHERE email = '{email}'")
    cur.execute("SELECT * FROM emails WHERE email = ?", (email,))
    return cur.fetchall()

  def get_format(self, cur, logger: Log, format):
    if not(logger is None):
      logger.debug(f"SELECT * FROM formatos WHERE id = {format}")
    cur.execute("SELECT * FROM formatos WHERE id = ?", (format,))
    return cur.fetchall()

  def get_format_name(self, cur, logger: Log, format):
    if not(logger is None):
      logger.debug(f"SELECT * FROM formatos WHERE UPPER(TRIM(name)) = '{format}'")
    cur.execute("SELECT * FROM formatos WHERE UPPER(TRIM(name)) = ?", (format,))
    return cur.fetchall()

  def get_predict_data(self, cur, logger: Log, format):
    if not(logger is None):
      logger.debug(f"SELECT * FROM coordenadas WHERE formato = {format} AND isRow = 0")
      logger.debug(f"SELECT * FROM columns WHERE formato = {format}")
  ...
  def from_dic_to_sql_data(self, logger: Log, data):
    fields = ', '.join(data.keys())
    num_fields = ', '.join('?' * len(data))
    values = tuple(data.values())
    return (fields, num_fields, values)
  
  def from_dic_to_sql_data_date(self, logger: Log, data):
  ...
  def insert_factura(self, cur, logger: Log, data):
    fields, num_fields, values = self.from_dic_to_sql_data_date(data)
    if not(logger is None):
      logger.info(f"INSERT INTO facturas ({fields}, datetime) OUTPUT INSERTED.id VALUES {values}")
    cur.execute(f"INSERT INTO facturas ({fields}, datetime) OUTPUT INSERTED.id VALUES ({num_fields})", values)
    return cur.fetchone()[0]
      

  def insert_detalles(self, cur, logger: Log, detalles):
    for i, detalle in enumerate(detalles):
      fields, num_fields, values = self.from_dic_to_sql_data(detalle)
      if not(logger is None):
        logger.info(f"INSERT INTO detalles ({fields}) VALUES {values}")
      cur.execute(f'INSERT INTO detalles ({fields}) VALUES ({num_fields})', values)
    return True
  
  def insert_format(self, cur, logger: Log, data):
    if not isEnvDev:
      name, height, width = data

      if not(logger is None):
        logger.debug(f"""IF EXISTS (SELECT 1 FROM formatos WHERE name = '{name}')
      BEGIN
  ...
  def get_formats(self, cur, logger: Log = None, filter=True):
    if not(logger is None):
      logger.debug("SELECT * FROM formatos")
  ...
  def get_formatid_by_name(self, cur, logger: Log, name):
    if not(logger is None):
      logger.debug(f"SELECT * FROM formatos WHERE name = '{name}'")
    cur.execute("SELECT id FROM formatos WHERE name = ?", (name, ))
    return cur.fetchone()[0]
  
  def get_columns_by_format(self, cur, logger: Log, id):
    cur.execute("SELECT * FROM columns WHERE formato = ?", (id,))
    return cur.fetchall()
  
  def get_coordinates_by_format(self, cur, logger: Log, id):
    if not(logger is None):
      logger.debug(f"SELECT * FROM coordenadas WHERE formato = {id}")
  ...
  def get_columns_for_editing(self, cur, logger: Log, id):
    if not(logger is None):
      logger.debug(f"SELECT originalx AS x, originaly AS y, width, height, field, [file], imageName FROM columns WHERE formato = {id}")
  ...
  def get_format_rows(self, cur, logger: Log, name):
    if not(logger is None):
      logger.debug(f"SELECT c.* FROM coordenadas c JOIN formatos f ON c.formato = f.id WHERE f.name = '{name}' AND c.isRow = 1")
    cur.execute("SELECT c.* FROM coordenadas c JOIN formatos f ON c.formato = f.id WHERE f.name = ? AND c.isRow = 1", (name,))
    return cur.fetchall()

  def get_rows_roi(self, cur, logger: Log, name):
    if not(logger is None):
      logger.debug(f"SELECT c.* FROM coordenadas c JOIN formatos f ON c.formato = f.id WHERE f.name = '{name}' AND c.field = 'filas'")
  ...
  def get_rows_roi_id(self, cur, logger: Log, name):
    if not(logger is None):
      logger.debug(f"SELECT c.* FROM coordenadas c JOIN formatos f ON c.formato = f.id WHERE f.id = {name} AND c.field = 'filas'")
    cur.execute("SELECT c.* FROM coordenadas c JOIN formatos f ON c.formato = f.id WHERE f.id = ? AND c.field = 'filas'", (name,))
    coords = cur.fetchone()
    return (int(coords[1]), int(coords[1]) + int(coords[3]), int(coords[0]), int(coords[0]) + int(coords[2]))
    

  def delete_column_by_field(self, cur, logger: Log, field, format):
    if not(logger is None):
      logger.debug(f"DELETE FROM columns WHERE field = '{field}' AND formato = {format}")
    cur.execute("DELETE FROM columns WHERE field = ? AND formato = ?", (field, format))

  def delete_coordinate_by_field(self, cur, logger: Log, field, format):
    if not(logger is None):
      logger.debug(f"DELETE FROM coordenadas WHERE field = '{field}' AND formato = {format}")
    cur.execute("DELETE FROM coordenadas WHERE field = ? AND formato = ?", (field, format))

  def insert_email(self, cur, logger: Log, data):
    if not isEnvDev:
      email, value = data
  ...
  def insert_coordinate(self, cur, logger: Log, data):
    if not isEnvDev:
  ...
  def insert_column(self, cur, logger: Log, data):
    if not isEnvDev:
  ...
  def update_format_state(self, cur, logger: Log, id):
    if not(logger is None):
      logger.debug(f"UPDATE formatos SET estado = '1' WHERE id = {id}")
    cur.execute("UPDATE formatos SET estado = '1' WHERE id = ?", (int(id),))

  def update_factura(self, cur, logger: Log, id, field, value):
    if not(logger is None):
      logger.debug(f"UPDATE facturas SET {field} = '{value}' WHERE id = {id}")
